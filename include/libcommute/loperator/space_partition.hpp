...
*/
template <typename HSType> class space_partition {
...
public:
  ...
  sv_index_type dim() const { return hs.dim(); }

  // Number of subspaces
  sv_index_type n_subspaces() const { return root_to_subspace.size(); }
  ...
  template <typename LOpScalarType, int... LOpAlgebraIDs>
  connections_map
  find_connections(loperator<LOpScalarType, LOpAlgebraIDs...> const& op) const {
    using scalar_type =
        typename loperator<LOpScalarType, LOpAlgebraIDs...>::scalar_type;
  ...
  // Apply a functor `f` to all basis states in a given Hilbert space.
  // The functor must take two arguments, index of the basis state, and index
  // of the subspace this basis state belongs to within a given space partition.
  template <typename F> friend void foreach(space_partition const& sp, F&& f) {
    foreach(sp.hs,
            [&sp, &f](sv_index_type n) { std::forward<F>(f)(n, sp[n]); });
  }
...
private:
  void update_root_to_subspace() {
    ...
    foreach(hs, [this](sv_index_type n) {
      root_to_subspace.emplace(ds.find_root(n), root_to_subspace.size());
    });
  }
};

// Factory functions for space_partition
