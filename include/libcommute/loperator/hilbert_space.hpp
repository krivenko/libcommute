...
public:
  ...
  sparse_foreach_basis_state& operator=(sparse_foreach_basis_state const& hs) = default;
  sparse_foreach_basis_state& operator=(sparse_foreach_basis_state&&) noexcept = default;
  ~sparse_foreach_basis_state() = default;

  template<typename Function>
  inline void operator()(Function && f) const {
    ...
    for(sv_index_type i = 0; i < n_states_; ++i) {
      ...
      for(std::size_t d = 0;
          d < indices_.size() - 1 && indices_[d] == sizes_[d]; ++d) {
        indices_[d] = 0;
        ++indices_[d + 1];
        state += jumps_[d];
      }
  ...
  hilbert_space(hilbert_space const& hs)
    : bit_range_end_(hs.bit_range_end_),
      ...
      foreach_alg_(hs.foreach_alg_ ?
        make_unique<detail::sparse_foreach_basis_state>(*hs.foreach_alg_) : nullptr
      ) {
