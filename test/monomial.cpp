...
TEST_CASE("Monomials", "[monomial]") {
  ...
  using gen_ptr_type = std::shared_ptr<const gen_type>;
  ...
  SECTION("Constructors") {
    auto m = mon_type(*Cdag_dn, *Sp_i);
    CHECK(mon_type{Cdag_dn, Sp_i} == m);
    CHECK(mon_type(std::vector<gen_ptr_type>{Cdag_dn, Sp_i}) == m);
  }

  std::vector<gen_ptr_type> basis_gens = {Cdag_dn, A_y, Sp_i, S1z_j};
