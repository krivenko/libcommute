...
TEST_CASE("Multiplication", "[mul]") {
  SECTION("double") {
    ...
    CHECK_THAT((real::a(0, "x") * expr_r),
               Prints<ref_t>("1*C+(1,up)C(2,dn)A(0,x)"));
    CHECK_THAT((expr_r * real::c_dag(1, "up")), Prints<ref_t>("0"));
    CHECK_THAT((real::c_dag(1, "up") * expr_r), Prints<ref_t>("0"));
  }
